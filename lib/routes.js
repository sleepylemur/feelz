...
module.exports = function(app, Db, io) {
  ...
  app.get('/api/users/:user_id', function(req,res){
    db.many("SELECT posts.id, username, avatar_image_url, timestamp, location_name, message, emotion, lat, lng, image_url FROM posts JOIN users ON posts.user_id = users.id WHERE users.id = $1 ORDER BY posts.timestamp DESC", req.params.user_id).then(function(data){
      res.json({data:data});
    });
  });
