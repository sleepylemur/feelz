...
module.exports = function(app, Db, io) {
  ...
  app.get('/api/users/:user_id', function(req,res){
    db.many("SELECT posts.id, users.username, users.avatar_image_url, posts.timestamp, posts.location_name, posts.message, posts.emotion, posts.lat, posts.lng, posts.image_url FROM users LEFT JOIN posts ON posts.user_id = users.id WHERE users.id = $1 ORDER BY posts.timestamp DESC", req.params.user_id).then(function(data){
      res.json({data:data});
    });
  });
