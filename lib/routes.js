...
module.exports = function(app, Db, io) {
  ...
  app.get('/api/users/:user_id', function(req,res){
    ...
    db.many("SELECT posts.id, users.username, "+returnemail+"users.avatar_image_url, posts.timestamp, posts.location_name, posts.message, posts.emotion, posts.lat, posts.lng, posts.image_url FROM users LEFT JOIN posts ON posts.user_id = users.id WHERE users.id = $1 ORDER BY posts.timestamp DESC", req.params.user_id).then(function(data){
      res.json({data:data});
    });
  });
  ...
  app.put('/api/users/:user_id', function(req,res) {
    if (req.params.user_id != req.user.id) {
      res.status(401).send('Need to be logged in as the user you want to update');
    } else {
      db.none('UPDATE users SET avatar_image_url = $1, username = $2, email = $3 WHERE id = $4', [req.body.avatar_image_url, req.body.username, req.body.email, req.user.id]).then(function() {
