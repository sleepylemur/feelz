...
module.exports = function(app, Db, io) {
  ...
  app.get('/api/profile', function(req,res){
    var targerUserId = (req.query.id!= undefined) ? parseInt(req.query.id) : req.user.id;

    db.many("SELECT username, avatar_image_url, timestamp, location_name, message, emotion, lat, lng, image_url FROM posts JOIN users ON posts.user_id = users.id WHERE users.id = $1 ORDER BY posts.timestamp DESC", targerUserId).then(function(data){
      res.json({data:data});
    });
  });
