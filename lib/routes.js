...
module.exports = function(app, Db) {
  ...
  app.post('/signup', function(req, res){
    db.none("SELECT * FROM users WHERE email = $1", req.body.email).then(function() {
      ...
      Db.digestPassword(req.body.password).then(function(password_digest) {
        return db.one("INSERT INTO users (email,password) VALUES ($1,$2)", req.body.email, password_digest);
      }).then(function(user) {
        var token  = jwt.sign({id: user.id}, jwtsecret, {expiresInMinutes: 60});
        res.json({token:token, id: user.id})
      }).catch(function(err) {
  ...
  app.post('/api/posts', function(req, res){
    console.log('/api/posts',req.body);
    db.one("INSERT INTO posts (timestamp, emotion, message, lat, lng, user_id) VALUES ($1,$2,$3,$4,$5,$6)",
      new Date(),req.body.emotion, req.body.message, req.body.location.lat, req.body.location.lng, req.user.id)
      .then(function(post) {
        console.log(post);
        res.json(post);
      }).catch(function(err) {
