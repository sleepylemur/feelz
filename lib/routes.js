...
module.exports = function(app, Db, io) {
  ...
  function checkDbPurge() {
    if (dbCounters.votes+dbCounters.users+dbCounters.posts > 100) purgeDb();
  }
  ...
  app.post('/signup', function(req, res){
    db.none("SELECT * FROM users WHERE email = $1", req.body.email).then(function() {
      ...
      Db.digestPassword(req.body.password).then(function(password_digest) {
        return db.one("INSERT INTO users (email,username,password) VALUES ($1,$2,$3) RETURNING id", [req.body.email, req.body.username, password_digest]);
      }).then(function(user) {
        dbCounters.users++;
        var token  = jwt.sign({id: user.id}, jwtsecret, {expiresInMinutes: 60});
        res.json({token:token, id: user.id})
