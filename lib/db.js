  ...
  this.reset = function reset() {
    return this.db.tx(function() {
      ...
      return Promise.all([
        ...
        return ctx.none("CREATE TABLE posts (id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES users ON DELETE CASCADE, timestamp TIMESTAMPTZ, lat DECIMAL, lng DECIMAL, message TEXT, emotion TEXT, image_url TEXT, location_name TEXT)");
      }).then(function() {
        // console.log('posts table created');
        return ctx.none("CREATE TABLE votes (id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES users ON DELETE CASCADE, post_id INTEGER REFERENCES posts ON DELETE CASCADE, timestamp TIMESTAMPTZ)");
      }).then(function() {
