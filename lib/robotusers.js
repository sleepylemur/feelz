...
function Robot() {
  ...
  this.origlng = -74.0059593 + Math.random()*0.01;
  this.lat = this.origlat;
  this.lng = this.origlng;
  this.speed = 0.003;
  this.turnspeed = 0.1;
  this.homingspeed = 0.001;
  ...
  this.ping = function() {
    if (this.token) {
      ...
      var image_url;
      if (Math.random() < 0.4) image_url = "";
      else image_url = robotimages[Math.floor(Math.random()*robotimages.length)];
      var emotions = ["rave", "rant"];
      var emo = emotions[Math.round(Math.random())];
